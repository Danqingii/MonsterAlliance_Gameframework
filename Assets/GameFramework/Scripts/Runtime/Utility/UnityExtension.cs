...
public static class UnityExtension
{
    ...
    /// <param name="isActive">子物体是否活跃</param>
    public static Transform FindChild(this Transform t, string childName,bool isActive = false)
    {
        Transform[] allChilid = t.GetComponentsInChildren<Transform>(isActive);

        for (int i = 0; i < allChilid.Length; i++)
        {
            if (allChilid[i].name == childName)
            {
                return allChilid[i];
            }
        }
        Log.Error("Find '{0}' Child  Error.",childName);
        return null;
    }
    ...
    /// <param name="childName">子物体名字</param>
    /// <param name="isActive">子物体是否活跃</param>
    public static T GetComponent<T>(this Transform t,string childName,bool isActive = false) where  T :Component
    {
        Transform child = FindChild(t,childName,isActive);
        if (child != null)
        {

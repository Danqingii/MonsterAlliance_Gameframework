...
namespace GameFramework.Download
{
    ...
    {
        /// <summary>
        /// 下载代理。(调用了 IDownloadAgentHelper实现类 完成真正的下载)
        /// </summary>
        private sealed class DownloadAgent : ITaskAgent<DownloadTask>, IDisposable
        {
            ...
            public GameFrameworkAction<DownloadAgent> DownloadAgentStart;              //开始
            public GameFrameworkAction<DownloadAgent, int> DownloadAgentUpdate;        //更新
            public GameFrameworkAction<DownloadAgent, long> DownloadAgentSuccess;      //成功
            public GameFrameworkAction<DownloadAgent, string> DownloadAgentFailure;    //失败
            ...
            {
                //初始化下载4个代理事件 更新数据流 更新数据大小 下载完成  错误
            ...
            public StartTaskStatus Start(DownloadTask task)
            {
                ...
                try
                {
                    if (File.Exists(downloadFile))
                    {
                        ...
                        //将此流的当前位置设置为给定值。
                        //有两个参数 第一参数规定文件指针以字节为单位移动的距离。第二个参数规定开始计算的位置
            ...
            public void Dispose()
            {
                ...
                //就是说当你手动调用Dispose或者Close方法释放了非托管资源后，
                //通过此方法强制告诉CLR不要再触发我的析构函数了，否则再执行析构函数相当于又做了一次清理非托管资源的操作，造成未知风险。
            }

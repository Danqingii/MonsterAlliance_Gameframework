...
namespace GameFramework.ObjectPool
{
    internal sealed partial class ObjectPoolManager : GameFrameworkModule, IObjectPoolManager
    {
        ...
        {
            private readonly GameFrameworkMultiDictionary<string, Object<T>> m_Objects;            //对象池字典 key=name value=对象类
            private readonly Dictionary<object, Object<T>> m_ObjectMap;                            //对象池字典 key=对象 value=对象类
            private readonly ReleaseObjectFilterCallback<T> m_DefaultReleaseObjectFilterCallback;
            private readonly List<T> m_CachedCanReleaseObjects;                                    //对象池中能释放的对象缓存
            private readonly List<T> m_CachedToReleaseObjects;                                     //对象池中能
            private readonly bool m_AllowMultiSpawn;  //获取是否允许对象被多次获取。
            private float m_AutoReleaseInterval;      //获取或设置对象池自动释放可释放对象的间隔秒数。
            private int m_Capacity;                   //获取或设置对象池的容量。
            private float m_ExpireTime;               //获取或设置对象池对象过期秒数。
            private int m_Priority;                   //对象池的优先级
            private float m_AutoReleaseTime;          //自动释放池时间
            ...
            public void Unspawn(object target)
            {
                ...
                if (internalObject != null)
                {
                    ...
                    //如果拥有的数量已经大于了对象池容量 && 当前没有获取任何对象 可以重置一下对象池了
            ...
            public bool ReleaseObject(object target)
            {
                ...
                //如果对象真正被使用中   或者已经被加锁   或者有自定义的释放检查标记 都是不释放的

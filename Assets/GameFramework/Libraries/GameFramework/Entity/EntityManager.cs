...
namespace GameFramework.Entity
{
    ...
    {
        private readonly Dictionary<int, EntityInfo> m_EntityInfos;        //全部的实体信息字典 其实就是所有的Group 里面的小成员
        private readonly Dictionary<string, EntityGroup> m_EntityGroups;   //全部的实体组字典
        private readonly Dictionary<int, int> m_EntitiesBeingLoaded;       //
        private readonly HashSet<int> m_EntitiesToReleaseOnLoad;           //
        private readonly Queue<EntityInfo> m_RecycleQueue;                 //需要回收的实体信息类(信息类中 持有真正的实体) 在轮询时释放
        private readonly LoadAssetCallbacks m_LoadAssetCallbacks;          //加载资源的回调 其中有 (加载成功)(加载失败)(加载更新)(加载依赖)4个回调
        private IObjectPoolManager m_ObjectPoolManager;
        private IResourceManager m_ResourceManager;
        private IEntityHelper m_EntityHelper;
        private int m_Serial;          //连续的编号
        private bool m_IsShutdown;     //是否关闭实体管理器
        ...
        public void DetachEntity(int childEntityId, object userData)
        {
            ...
            parentEntityInfo.RemoveChildEntity(childEntity);   //先从父实体出发 自身是子实体 子实体从集合中移除
            parentEntity.OnDetached(childEntity, userData);    //父实体 解除 跟子实体的 引用
            childEntity.OnDetachFrom(parentEntity, userData);  //子实体 解除 跟父实体的 引用
        }
        ...
        private void InternalShowEntity(int entityId, string entityAssetName, EntityGroup entityGroup, object entityInstance, bool isNewInstance, float duration, object userData)
        {
            try
            {
                ...
                //创建所属该实体的 信息类 完成赋值 执行初始化回调 执行刷新回调
        ...
        private void InternalHideEntity(EntityInfo entityInfo, object userData)
        {
            ...
            IEntity entity = entityInfo.Entity;
            DetachEntity(entity.Id, userData);           //解除关系 也就是把自己跟 子父 实体的关系解除 删除他们引用
            ...
            entityGroup.RemoveEntity(entity);             //把实体从该组中移除
            if (!m_EntityInfos.Remove(entity.Id)) 
            {
                throw new GameFrameworkException("Entity info is unmanaged.");
            }

            if (m_HideEntityCompleteEventHandler != null) //执行关闭实体回调
            {
            ...
            m_RecycleQueue.Enqueue(entityInfo);           //把实体信息类 放入信息回收池中 准备回收引用
        }

...
namespace GameFramework
{
    ...
    {
        private const int BlockSize = 1024 * 4;      //块大小
        private static byte[] s_CachedBytes = null;  //缓冲二进制流的大小。

        private readonly T m_Owner;
        private readonly LoadAssetCallbacks m_LoadAssetCallbacks;    //加载资源回调  里面有成功 失败 更新 回调函数
        private readonly LoadBinaryCallbacks m_LoadBinaryCallbacks;  //加载二进制文件回调
        private IResourceManager m_ResourceManager;
        private IDataProviderHelper<T> m_DataProviderHelper;
        private EventHandler<ReadDataSuccessEventArgs> m_ReadDataSuccessEventHandler;                  //读取数据成功
        private EventHandler<ReadDataFailureEventArgs> m_ReadDataFailureEventHandler;                  //读取数据失败
        private EventHandler<ReadDataUpdateEventArgs> m_ReadDataUpdateEventHandler;                    //读取属性更新
        private EventHandler<ReadDataDependencyAssetEventArgs> m_ReadDataDependencyAssetEventHandler;  //读取数据依赖

...
namespace GameFramework
{
    ...
    {
        private readonly Dictionary<byte, SerializeCallback> m_SerializeCallbacks;        //保存所有的序列化回调
        private readonly Dictionary<byte, DeserializeCallback> m_DeserializeCallbacks;    //保存所有的  通过指定流得到反序列化的数据回调
        private readonly Dictionary<byte, TryGetValueCallback> m_TryGetValueCallbacks;    //保存所有的  尝试从指定键+指定流  得到一个回调函数
        private byte m_LatestSerializeCallbackVersion;   //最新的回调函数版本
